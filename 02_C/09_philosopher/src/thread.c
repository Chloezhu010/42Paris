...
/*
    ...
    - used to determine the philo_routine activity, prevent further action if he died
*/
...
int create_thread(t_program *program, pthread_mutex_t *fork)
{
    ...
    if (pthread_create(&observer, NULL, &monitor, program->philo) != 0)
        destory_all(program, fork, "Error thread cannot be created");
    i = 0;
    while (i < program->philo->num_philo)
    {
        if (pthread_create(&program->philo[i].thread, NULL, &philo_routine, &program->philo[i]))
            destory_all(program, fork, "Error thread cannot be created");
        i++;
    }
    if (pthread_join(observer, NULL) != 0)
        destory_all(program, fork, "Error thread cannot be joined");
    i = 0;
    while (i < program->philo->num_philo)
    {
        if (pthread_join(program->philo->thread, NULL) != 0)
            destory_all(program, fork, "Error thread cannot be joined");
        i++;
    }
    return (0);
